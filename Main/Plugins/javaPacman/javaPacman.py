...
from handler import JVMHandler
...
def readCommand(argv):
    ...
    if args["layout"] == None:
        raise Exception("The layout " + options.layout + " cannot be found. Ensure it exists in the layouts subdirectory.")
...
def runGames(
    ...
    # we start and stop the jvm here
    # TODO: user provided JVM path
...
if __name__ == "__main__":
    ...
    for i in range(len(args)):
        if skip_next:
            # Skip the current argument because it's the value after --jpype-dir
            jpype_dir = args[i]
            skip_next = False
            continue
        ...
        if args[i] == "--jpype-dir":
            ...
            # Otherwise, add the argument to the filtered list
            filtered_args.append(args[i])
    
    args = filtered_args 
       
    jpype_dir = os.path.dirname(jpype_dir)
    ...
    s.main(jpype.JArray(jpype.JString)(sys.argv[1:]))
