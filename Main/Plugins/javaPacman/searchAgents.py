    ...
    def __init__(self, startingGameState):
        ...
        for corner in self.corners:
            if not startingGameState.hasFood(*corner):
                print("[python] Warning: no food in corner " + str(corner))
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def __init__(self):
        self.ClosestDotSearchAgent = jpype.JClass("capstone.prototype.problem.ClosestDotSearchAgent")()

    def registerInitialState(self, state):
        from pyToJava import JavaConverter

        jGameState = JavaConverter.py_gamestate_to_java(state)
        self.ClosestDotSearchAgent.registerInitialState(jGameState)

        self.actions = []

        # convert actions to python list
        jActions = self.ClosestDotSearchAgent.actions

        for i in range(jActions.size()):
            direction = jActions.get(i)

            if direction.toString() == "NORTH":
                self.actions.append(Directions.NORTH)
            elif direction.toString() == "SOUTH":
                self.actions.append(Directions.SOUTH)
            elif direction.toString() == "EAST":
                self.actions.append(Directions.EAST)
            elif direction.toString() == "WEST":
                self.actions.append(Directions.WEST)
            else:
                raise Exception("Invalid direction: " + direction.toString())
    ...
    def findPathToClosestDot(self, gameState):
        ...
        gameState.
        """
        raise Exception("Not implemented, use Java implementation")
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def isGoalState(self, state):
        ...
        """
        x, y = state
        conv = JavaConverter(self)

        J_AnyFoodSearchProblem = jpype.JClass(
            "capstone.prototype.problem.AnyFoodSearchProblem"
        )
        Position = jpype.JClass("capstone.prototype.types.Position")
        PositionSearchProblemState = jpype.JClass(
            "capstone.prototype.problem.states.PositionSearchProblemState"
        )

        jPos = Position(jpype.JDouble(x), jpype.JDouble(y))
        jPosSearchProblemState = PositionSearchProblemState(jPos)
        jGameState = conv.get_game_state()
        jProblem = J_AnyFoodSearchProblem(jGameState)

        result = jProblem.isGoalState(jPosSearchProblemState)
        return result
